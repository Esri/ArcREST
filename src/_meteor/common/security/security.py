...
try:
    ...
    hasNTLM = False
#from . import _abstract as abstract
...
from ..web._con import _connection


########################################################################
class LDAPSecurityHandler(object):
    """
       This Security Handler handles LDAP based security.
    """
    _defaultTokenExpiration = 1440
    _jar = None
    _handler = None
    ...
    @property
    def is_portal(self):
        if self._portalSelf is None:
            self.check_portal()
        return self._is_portal
    #----------------------------------------------------------------------
    def check_portal(self):
        self._portalSelf = self._con.get(url=self._org_url + "/sharing/rest/portals/self",
                                       param_dict={'f' : 'json'},
                                       securityhandler=self)
        self._is_portal = self._portalSelf['isPortal']
    #----------------------------------------------------------------------
    @property
    def method(self):
    ...
    def loadusername(self):
        if self._username is None:
            if self._portalSelf is None:
                self.is_portal
            if 'username' in self._portalSelf['user']:
                self._username = self._portalSelf['user']['username']
            else:
    ...
    def portalServerHandler(self, serverUrl, username=None):
        ...
        info = self._con.get(url=url,
                             param_dict={'f' : 'json'})
        if "tokenServicesUrl" in info['authInfo']:
            token_url = info['authInfo']['tokenServicesUrl']

        if username is None:
            username = self._login_username

        ptsh = PortalTokenSecurityHandler(connection=self._con,
                                          username=username,
        ...
        pssh = PortalServerSecurityHandler(connection=self._con,
                                           tokenHandler=ptsh,
                                           serverUrl=serverUrl,
...
class NTLMSecurityHandler(LDAPSecurityHandler):
    ...
    _defaultTokenExpiration = 1440
    _con = None
    def __init__(self, connection, org_url, username, password,referer_url=None):
        self._con = connection
        self._login_username = username
        self._password = password
    ...
    def cookiejar(self):
        ...
        return self._jar
########################################################################
class PKISecurityHandler(object):
    """
    ...
    def __init__(self,
                 connection,
                 org_url, keyfile, certificatefile,
                 proxy_url=None, proxy_port=None, referer_url=None):
        """Constructor"""
        self._con = connection
    ...
    #----------------------------------------------------------------------
    def check_portal(self):
        portalSelf = self._con.get(url=self._org_url + "/sharing/rest/portals/self",
                                       param_dict={'f' : 'json'},
                                       securityhandler=self)
        self._is_portal = portalSelf['isPortal']
    ...
    class HTTPSClientAuthHandler(request.HTTPSHandler):
        ...
        def getConnection(self, host, timeout=300):
            return  self.httplib.HTTPSConnection(host,
                                                 ...
                                                 timeout=timeout)
########################################################################
class PortalServerSecurityHandler(object):
    """
    ...
    def is_portal(self):
        return False
########################################################################
class OAuthSecurityHandler(object):
    ...
    #----------------------------------------------------------------------
    def __init__(self, connection,
                 client_id, secret_id, org_url,token_url=None):
        """Constructor"""
        self._con = connection
        ...
        self._token_expires_on = datetime.datetime.now() + datetime.timedelta(seconds=self._defaultTokenExpiration)
    ...
    def _generateForOAuthSecurity(self, client_id,
        ...
        if 'access_token' in token:
            ...
            #self._token_expires_on = None
########################################################################
class ArcGISTokenSecurityHandler(object):
    ...
    _token_url = None
    #----------------------------------------------------------------------
    def __init__(self, connection, proxy_url=None, proxy_port=None):
        """Constructor"""
        self._con = connection
        if arcpyFound == False:
    ...
    def _generateForServerTokenSecurity(self,
        ...
        query_dict = {'serverURL':serverURL,
                      'token': token,
                      'expiration':str(self._defaultTokenExpiration),
                      'f': 'json',
                      'request':'getToken'}
...
#########################################################################
class AGSTokenSecurityHandler(object):
    ...
    def __init__(self, connection,
        ...
        if token_url is None and \
           not org_url is None:
            ...
            result = self._con.get(url=url, param_dict=params)
    ...
    def _generateForTokenSecurity(self,
        ...
        if "token" not in token:
            ...
            return token['token']
########################################################################
class PortalTokenSecurityHandler(object):
    """
...
########################################################################
class AGOLTokenSecurityHandler(object):
    ...
    def __init__(self,
        ...
        self._token_url = token_url
        self._org_url = org_url
        self._token_expires_on = datetime.datetime.now() + datetime.timedelta(seconds=60)

        urlInfo = urlparse(org_url)
        if str(urlInfo.netloc).lower() == "www.arcgis.com"> -1:

            self._initURL(org_url=org_url,token_url=token_url)
            if isinstance(self._con, _connection): pass
            portalSelf = self._con.get(url=org_url + "/sharing/rest/portals/self",
                                       param_dict={'f' : 'json'},
                                       securityhandler=self)
            baseURL = "{}.{}".format(portalSelf['urlKey'], portalSelf['customBaseUrl'])
    ...
    def _initURL(self, org_url=None,
        ...
        if token_url is None:
            results = self._con.get(url= self._surl + '/info',
                                   param_dict={'f':'json'})
            if 'authInfo' in results and 'tokenServicesUrl' in results['authInfo']:
...
########################################################################
class TokenSecurityHandler(object):
    """ handles Server/Portal/ArcGIS Online Token Base Security
        username - required - username to access AGOL services
        password - required - password for username above
        org_url - The url of that ArcGIS Organization.  This url is
         composed on the machine name and the instance name of the portal.
    ...
    def __init__(self,
        ...
        if token_url:
            self._token_url = token_url
        elif org_url == self._org_url and token_url is None:
            self._token_url = self._default_token_url
        self._username = username
        self._password = password
        self._org_url = org_url
        self.__init()
        #http://chronus.esri.com/arcgis/rest/info?f=pjson -- Server
        #http://chronus.esri.com/portal/sharing/rest/info?f=pjson -- Portal
        #http://www.arcgis.com/sharing/rest/info?f=pjson -- AGOL
    #----------------------------------------------------------------------
    def __init(self):
        """"""

        # 1 get the site's info
        if self._token_url == self._default_token_url:
            self._is_agol = True
        elif self._token_url is None:
            urls = {
                "service" : "{org}/{part}".format(org=self._org_url, part="rest/info"),
                "agol/portal" : "{org}/{part}".format(org=self._org_url, part="sharing/rest/info")
            }
            for k,v in urls.items():
                res = self._con.get(url=v, param_dict={"f" : "json"},
                                    securityhandler=None,
                                    compress=True,
                                    out_folder=None,
                                    file_name=None)
                ...
                print (res)
                print ('stop')
        #if isinstance(self._con, _connection):
            #for k,v in urls.items():
                #res = self._con.get(url=v, param_dict={"f" : "json"},
                                     #securityhandler=None,
                                     #compress=True,
                ...
                #del k,v
        # 2 If AGOL/Portal get the Org URL
        #self._username = username
        #self._password = password
        #self._token_url = token_url
        #self._org_url = org_url
    ...
    #def __getRefererUrl(self, url=None):
        ...
        #params = {
            ...
            #"token" : self.token
        #}
        #val = self._con.get(url=url, param_dict=params,
                           #proxy_url=self._proxy_url,
                           #proxy_port=self._proxy_port)
